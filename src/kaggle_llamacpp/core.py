...
from .download import download_with_tqdm, extract_archive, filename_from_url
...
def ensure_llamacpp_cuda(*, paths: RuntimePaths | None = None, repo: str = "ai-dock/llama.cpp-cuda", cuda_version: str = "12.8", force: bool = False) -> dict:
    ...
    if existing and not force:
        ...
        download_with_tqdm(asset_url, archive, force=force)
    ...
    return write_state(llama_home=str(llama_home), llama_bin=str(llama_bin), llama_cli=str(llama_cli) if llama_cli else "", model_dir=str(paths.model_dir), port=8080)


def download_model(model_url: str, *, output_name: str | None = None, hf_token: str | None = None, paths: RuntimePaths | None = None, force: bool = False) -> Path:
    paths = paths or RuntimePaths()
    paths.make_dirs()
    ...
    path = download_with_tqdm(model_url, dest, token=token, force=force)
...
def start_llama_server(config: ServerConfig | None = None, *, paths: RuntimePaths | None = None) -> int:
    ...
    write_state(port=config.port, ctx_size=config.ctx_size, batch_size=config.batch_size, ubatch_size=config.ubatch_size, reasoning_format=config.reasoning_format, enable_thinking=config.enable_thinking, llama_pid=proc.pid, llama_log=str(paths.log_path))
...
def test_chat_completion(prompt: str = "Say hello in one short sentence.", *, port: int | None = None, max_tokens: int = 64, temperature: float = 0.7, clean: bool = True) -> str:
    state = read_state()
    port = port or int(state.get("port", 8080))
    payload = {"model": "local", "messages": [{"role": "user", "content": prompt}], "max_tokens": max_tokens, "temperature": temperature}
