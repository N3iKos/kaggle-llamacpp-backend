...
def ensure_ngrok(ngrok_dir: str | Path = "/kaggle/working/ngrok") -> Path:
    ...
    if not ngrok_bin.exists():
        ...
        download_file(NGROK_URL, archive, backend="aria2")
...
def configure_ngrok_token(
    *,
    secret_name: str = "NGROK_AUTHTOKEN",
    ngrok_bin: str | Path = "/kaggle/working/ngrok/ngrok",
) -> str:
...
def start_ngrok_tunnel(
    *,
    port: int = 8080,
    secret_name: str = "NGROK_AUTHTOKEN",
    ngrok_dir: str | Path = "/kaggle/working/ngrok",
    log_path: str | Path = "/kaggle/working/ngrok.log",
    pid_path: str | Path = "/kaggle/working/ngrok.pid",
    timeout_s: int = 60,
) -> str:
    ...
    print(tail(log_path, 120))
    raise TimeoutError(f"Tidak menemukan HTTPS tunnel dalam {timeout_s}s.")
